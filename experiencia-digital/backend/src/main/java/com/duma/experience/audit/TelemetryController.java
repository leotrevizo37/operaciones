...
public class TelemetryController {
  ...
  public TelemetryController(SystemLogService systemLogService) {
    this.systemLogService = systemLogService;
  }

  @PostMapping
  public ResponseEntity<Void> record(
      @Valid @RequestBody Request body, Authentication authentication, HttpServletRequest request) {
    if (!EVENTS.contains(body.eventName()) || !OUTCOMES.contains(body.outcome())) {
      return ResponseEntity.unprocessableEntity().build();
    }
    String actor = authentication == null ? "standalone-read" : authentication.getName();
    systemLogService.recordClientEvent(
        body.eventName(), body.outcome(), actor, body.tenantId(), body.durationMs(), request);
