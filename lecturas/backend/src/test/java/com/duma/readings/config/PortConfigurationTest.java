...
class PortConfigurationTest {
  ...
  void resolvesCanonicalPortsIntoServerUrlIssuerAndCors() throws Exception {
    String modulePort = System.getProperty("DUMA_READINGS_BACKEND_PORT");
    String shellPort = System.getProperty("DUMA_SHELL_BACKEND_PORT");
    String frontendPort = System.getProperty("DUMA_READINGS_FRONTEND_PORT");
    try {
      System.setProperty("DUMA_READINGS_BACKEND_PORT", "19082");
      System.setProperty("DUMA_SHELL_BACKEND_PORT", "19080");
      System.setProperty("DUMA_READINGS_FRONTEND_PORT", "15175");
      StandardEnvironment environment = environment();

      assertThat(environment.getProperty("server.port")).isEqualTo("19082");
      assertThat(environment.getProperty("duma.module.api-base-url"))
          .isEqualTo("http://localhost:19082");
      ...
      assertThat(environment.getProperty("duma.security.allowed-origins"))
          .contains("http://localhost:15175");
    } finally {
      restore("DUMA_READINGS_BACKEND_PORT", modulePort);
      restore("DUMA_SHELL_BACKEND_PORT", shellPort);
      restore("DUMA_READINGS_FRONTEND_PORT", frontendPort);
    }
  }
